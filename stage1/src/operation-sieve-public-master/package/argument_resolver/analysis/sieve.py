...
from argument_resolver.utils.confidence import compute_confidence
...
class SieveAnalysis(ScriptBase):
    ...
    def save_closure(self, sink, defn, closure):
        ...
        # Compute confidence score with ANI skip records and LLM tags
        ani_skip_records = getattr(closure.handler, 'ani_skip_records', [])
        llm_summary_tags = getattr(closure.handler, 'llm_summary_tags', [])
        llm_summary_locations = getattr(closure.handler, 'llm_summary_locations', [])
        summary_augmented = bool(llm_summary_tags)

        confidence_data = compute_confidence(
            likely_ranks, possible_ranks,
            ani_skip_records, llm_summary_tags
        )

        all_sources = {
            "sources": sources,
            "rank": rank,
            # Extended confidence fields
            "confidence": confidence_data["confidence"],
            "source_score": confidence_data["source_score"],
            "path_factor": confidence_data["path_factor"],
            "risk_bonus": confidence_data["risk_bonus"],
            "summary_augmented": summary_augmented,
            "summary_tags": list(set(llm_summary_tags)),
            "summary_locations": llm_summary_locations,
            "ani_skip_records": ani_skip_records,
        }
    ...
    def contains_external_input(closure: Closure):
        contains_external, valid_funcs, _ = SieveAnalysis.search_for_external_input(
    ...
    def search_for_external_input(
        ...
        for func in parent_functions:
            ret = SieveAnalysis.search_for_external_input(
...
def get_cli_args():
    ...
    llm_group = parser.add_argument_group(
        "LLM Enhancement", "Options for LLM-enhanced analysis"
    )
    ...
    llm_group.add_argument(
        ...
        dest="llm_api_key",
        default=None,
        help="API key for LLM service (or set LLM_API_KEY env var)",
    )
    ...
    llm_group.add_argument(
        ...
        dest="llm_api_base",
        default=None,
        help="API base URL for LLM service (or set LLM_API_BASE env var)",
    )
    ...
    llm_group.add_argument(
        ...
        dest="llm_model",
        default=None,
        help="Model name for LLM service (or set LLM_MODEL env var)",
    )
    ...
    llm_group.add_argument(
        ...
        type=int,
        default=30,
        help="Max LLM calls per binary (default: 30)",
    )
    ...
    llm_group.add_argument(
        ...
        type=int,
        default=5,
        help="Max LLM calls per trace chain (default: 5)",
    )

    llm_group.add_argument(
        "--ghidra-cache",
        dest="ghidra_cache_path",
        default=None,
        help="Path to pre-generated Ghidra decompilation JSON cache",
    )
    ...
    llm_group.add_argument(
        ...
        dest="ghidra_home",
        default=None,
        help="Path to Ghidra installation directory (or set GHIDRA_HOME env var)",
    )
...
def main():
    args = get_cli_args()
    if args.merge:
        SieveAnalysis.merge_execve(Path(args.bin_path), Path(args.result_path))
    else:
        args.__dict__.pop("merge")
        analyzer = SieveAnalysis(**args.__dict__)
        analyzer.analyze()
